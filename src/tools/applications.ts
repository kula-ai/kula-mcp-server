...
export function register(server: McpServer, client: KulaClient) {
  ...
  server.registerTool(
    ...
    async ({ application_id, page, limit, sort_by, sort_order, created_after, created_before, updated_after, updated_before }) => {
      try {
        const data = await client.get(`/v1/applications/${application_id}/notes`, { page, limit, sort_by, sort_order, created_after, created_before, updated_after, updated_before });
